...
(function($){
	...
	var keyPressVal;
	//var dataId_yearEstablished = ; //prod 47134549
	...
	function formatYearEstablished() {
		...
		var year = loader.getEngine().getDocument().getElementById(dataId_yearEstablished);
		year.on('value-change', function() {
			year.setValue(year.getValue());
			var json_obj = year.getValue();
			if (json_obj["value"].length == 1 && json_obj["value"] == 0) {
				var reg = /^0/gi;
				console.log('year length before --> ' + json_obj["value"].length);
				json_obj["value"] = json_obj["value"].replace(reg,'');
				console.log('year value --> ' + json_obj["value"]);
				console.log('year length after --> ' + json_obj["value"].length);
			}
			if (json_obj["value"].length > 4) {
				console.log('year value --> ' + json_obj["value"]);
			}
	...
	$("div[data-id='"+dataId_yearEstablished+"']").find("input") 
			...
			.on('keypress', function(e) {
				...
				if (year.val().length == 0) {
					...
					console.log('year.val().length --> ' + year.val().length);
					console.log('keyLimit --> ' + keyLimit);;		
				}
				if (year.val().length > 3) {
					console.log('sobra limit year.val()----' + year.val());
					console.log('sobra limit keyLimit----' + keyLimit);
					//year.val() = year.val().slice(0,4);
				}
     		});
     	*/
	}
